...
class CreateSchedulePage(ttk.Frame):
    ...
    def generate_random_schedule(self):
                ...
                for user_id in available_users:
                    ...
                    if user_id in user_availabilities:
                        for avail in user_availabilities[user_id]:
                            ...
                            if (
                                avail_start <= shift_start_time
                                and avail_end >= shift_end_time
                            ):
                                can_work_shift = True
                                break
        ...
        for shift in selected_shift_objects:
            day_index = (
                days_of_week.index(shift.day_of_week) + 2
            )  # +2 because first two columns are start/end times

            # Find the right row for this shift's time
            for row in data:
                if (
                    row[0] == shift.start_time
                    and row[1] == shift.end_time
                    and row[day_index] is None
                ):
        ...
        ttk.Button(
            ...
            command=lambda: self.choose_and_close(popup, data),
        ).pack(pady=10)
    ...
    def generate_managercentric_schedule(self):
        ...
        for day, day_shifts in shifts_by_day.items():
            ...
            users_available_on_day.sort(
                key=lambda x: 0 if x["role"] == "Manager" else 1
            )
            ...
            if shift.user_id:
                ...
                if user_info and len(user_info) > 1:
                    ...
                    if user_role == "Manager":
                        shift.employee = (
                            f"{user_info[1]} (M)"  # Add (M) indicator for managers
                        )
                    else:
                        shift.employee = user_info[1]
                else:
        ...
        for day, day_shifts in shifts_by_day.items():
            ...
            for user_id, shift_options in user_shift_options.items():
                if user_id not in assigned_users:
                    remaining_options = [
                        s for s in shift_options if s not in assigned_shifts
                    ]
            ...
            for user_id, available_shifts in sorted_users:
                if user_id in assigned_users:
                    continue
                unassigned_shifts = [
                    s for s in available_shifts if s not in assigned_shifts
                ]
